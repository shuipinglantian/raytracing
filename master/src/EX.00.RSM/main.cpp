...
private:
    ...
    static constexpr int rsmW = 256;
    static constexpr int rsmH = 256;

    void initialize() override
    {
        gbuffer_.initialize(window_->getClientSize());

        rsm.initialize({ rsmW, rsmH });
    ...
    void frame() override
    {
        ...
        rsm.setLight(light, lightViewProj);

        rsm.begin();
        for(auto &m : meshes_)
            rsm.render(m.vertexBuffer, m.world);
        rsm.end();
        ...
        lowres_indirect_.setRSM(rsm.getRSM(0), rsm.getRSM(1), rsm.getRSM(2));
        ...
        shade_.setRSM(rsm.getRSM(0), rsm.getRSM(1), rsm.getRSM(2));
...
{ 
	...
	win.title = L"EX.00.ReflectiveShadowMap";
	win.sampleCount = 1;
    ReflectiveShadowMapApplication(win).run();
}
