...
private:
    ...
    ShadowAlgorithmType algoType_ = ShadowAlgorithmType::NoShadow;
...
void ShadowMapApplication::frameNoShadow(const Light &light, bool gui)
{
    ...
    for(auto &mesh : meshes_)
        noShadow_.render(mesh.vertexBuffer, Mat4::identity());//identity是对角矩阵E
    noShadow_.end();
}
...
void ShadowMapApplication::updateCamera()
{
    ...
    if(!mouse_->isVisible())
    {
		Camera::UpdateParams parm;
		parm.front = keyboard_->isPressed('W');
		parm.left = keyboard_->isPressed('A');
		parm.right = keyboard_->isPressed('D');
		parm.back = keyboard_->isPressed('S');
		parm.up = keyboard_->isPressed(KEY_SPACE);
		parm.down = keyboard_->isPressed(KEY_LSHIFT);
		parm.cursorRelX = static_cast<float>(mouse_->getRelativePositionX());
		parm.cursorRelY = static_cast<float>(mouse_->getRelativePositionY());
		camera_.update(parm);
    }
...
Light ShadowMapApplication::getLight()
{
	...
	Light light1;
	light1.position = light_pos;
	light1.fadeCosBegin = 1;
	light1.direction = (light_dst - light_pos).normalize();
	light1.fadeCosEnd = 0.97f;
	light1.incidence = Float3(80);
    ...
    const std::string &filename) const
{
    ...
    Mesh mesh;
    mesh.vertexBuffer.initialize(vertices.size(), vertices.data());//10174个三角形和2个三角形
...
{
	WindowDesc win;
	win.clientSize = { 640, 480 };
	win.title = L"00.ShadowMap";
	win.sampleCount = 4;
    ShadowMapApplication(win).run();
}
