...
namespace
{
    ...
    std::map<GLFWwindow*, windowt*>  g_glfw_windows;

    bool find_window(GLFWwindow *glfw_window, windowt **win_ptr)
    {
...
windowt::windowt(const windowdesc_t &desc, bool maximized)
{
    ...
    {
    case windowdesc_t::d0s0:
        glfwWindowHint(GLFW_DEPTH_BITS, 0);
        glfwWindowHint(GLFW_STENCIL_BITS, 0);
        break;
    case windowdesc_t::d24s8:
        glfwWindowHint(GLFW_DEPTH_BITS, 24);
        glfwWindowHint(GLFW_STENCIL_BITS, 0);
        break;
    case windowdesc_t::d32s0:
    ...
    misc::fixed_scope_guard_t windowguard([&]
    {
...
keyboard_t *windowt::get_keyboard() noexcept
{
    return &impl_->keyboard;
}

mouse_t *windowt::get_mouse() noexcept
{
    return &impl_->mouse;
}

void windowt::do_events()
{
...
bool windowt::is_in_focus()
{
    return impl_->focus;
}

void windowt::wait_for_focus()
{
    if(is_in_focus())
        return;
...
bool windowt::get_close_flag() const noexcept
{
    return impl_->close;
}

void windowt::set_close_flag(bool flag) noexcept
{
    impl_->close = flag;
}

void windowt::set_maximized()
{
    glfwMaximizeWindow(impl_->window);
}

int windowt::get_framebuffer_width() const noexcept
{
    return impl_->framebuffer_size.x;
}

int windowt::get_framebuffer_height() const noexcept
{
    return impl_->framebuffer_size.y;
}

vec2i windowt::get_framebuffer_size() const noexcept
{
    return impl_->framebuffer_size;
}

float windowt::get_framebuffer_w_over_h() const noexcept
{
    return static_cast<float>(impl_->framebuffer_size.x)
                            / impl_->framebuffer_size.y;
}

void windowt::swap_buffers()
{
    glfwSwapBuffers(impl_->window);
}

bool windowt::get_vsync() const noexcept
{
    return impl_->vsync;
}

void windowt::set_vsync(bool vsync) noexcept
{
    impl_->vsync = vsync;
    glfwSwapInterval(vsync ? 1 : 0);
}

void windowt::use_default_viewport() const
{
    glViewport(0, 0, impl_->framebuffer_size.x, impl_->framebuffer_size.y);
}

bool windowt::_imgui() const noexcept
{
    return impl_->imgui;
}

void windowt::_close(bool close_flag)
{
    impl_->close = close_flag;
    if(impl_->close)
        event_mgr_.send(windowclose_event_t{});
}

void windowt::_resize()
{
    ...
    event_mgr_.send(windowresize_event_t{
        impl_->framebuffer_size.x,
        impl_->framebuffer_size.y
    });
}

void windowt::_focus(bool focus)
{
    impl_->focus = focus;
    event_mgr_.send(windowfocus_event_t{ focus });
}

void windowt::_key(keycode_t keycode, bool pressed)
{
    if(pressed)
...
void windowt::_char(uint32_t ch)
{
    impl_->keyboard._input_char(ch);
}

void windowt::_mb_down(mouse_button_t button)
{
    impl_->mouse._mb_down(button);
}

void windowt::_mb_up(mouse_button_t button)
{
    impl_->mouse._mb_up(button);
}

void windowt::_wheel_scroll(int offset)
{
