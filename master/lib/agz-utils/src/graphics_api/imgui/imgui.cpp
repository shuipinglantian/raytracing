...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
    ...
    if (g.IO.MouseClicked[1])
    {
        ...
        bool hovered_windowabove_modal = false;
        if (modal == NULL)
            hovered_windowabove_modal = true;
        for (int i = g.Windows.Size - 1; i >= 0 && hovered_windowabove_modal == false; i--)
        {
        ...
        ClosePopupsOverWindow(hovered_windowabove_modal ? g.HoveredWindow : modal, true);
    }
}
...
static void FindHoveredWindow()
{
    ...
    ImGuiWindow* hovered_windowignoring_moving_window = NULL;
    ...
    for (int i = g.Windows.Size - 1; i >= 0; i--)
    {
        ...
        if (hovered_windowignoring_moving_window == NULL && (!g.MovingWindow || window->RootWindow != g.MovingWindow->RootWindow))
            hovered_windowignoring_moving_window = window;
        if (hovered_window && hovered_windowignoring_moving_window)
            break;
    }

    g.HoveredWindow = hovered_window;
    g.HoveredRootWindow = g.HoveredWindow ? g.HoveredWindow->RootWindow : NULL;
    g.HoveredWindowUnderMovingWindow = hovered_windowignoring_moving_window;
}
...
static ImVec2 CalcWindowSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size)
{
    ...
    {
        ImGuiWindow* windowfor_height = window;
        new_size = ImMax(new_size, g.Style.WindowMinSize);
        new_size.y = ImMax(new_size.y, windowfor_height->TitleBarHeight() + windowfor_height->MenuBarHeight() + ImMax(0.0f, g.Style.WindowRounding - 1.0f)); // Reduce artifacts with very small windows
    }
    return new_size;
}
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    const float windowrounding = window->WindowRounding;
    const float windowborder_size = window->WindowBorderSize;
    if (window->Collapsed)
    {
        // Title bar only
        ...
        RenderFrame(title_bar_rect.Min, title_bar_rect.Max, title_bar_col, true, windowrounding);
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bg_col, windowrounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);
        }
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            window->DrawList->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max, title_bar_col, windowrounding, ImDrawCornerFlags_Top);
        }
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
            ...
            window->DrawList->AddRectFilled(menu_bar_rect.Min + ImVec2(windowborder_size, 0), menu_bar_rect.Max - ImVec2(windowborder_size, 0), GetColorU32(ImGuiCol_MenuBarBg), (flags & ImGuiWindowFlags_NoTitleBar) ? windowrounding : 0.0f, ImDrawCornerFlags_Top);
        ...
        if (!(flags & ImGuiWindowFlags_NoResize))
        {
            for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
            {
                const ImGuiResizeGripDef& grip = resize_grip_def[resize_grip_n];
                const ImVec2 corner = ImLerp(window->Pos, window->Pos + window->Size, grip.CornerPosN);
                window->DrawList->PathLineTo(corner + grip.InnerDir * ((resize_grip_n & 1) ? ImVec2(windowborder_size, resize_grip_draw_size) : ImVec2(resize_grip_draw_size, windowborder_size)));
                window->DrawList->PathLineTo(corner + grip.InnerDir * ((resize_grip_n & 1) ? ImVec2(resize_grip_draw_size, windowborder_size) : ImVec2(windowborder_size, resize_grip_draw_size)));
                window->DrawList->PathArcToFast(ImVec2(corner.x + grip.InnerDir.x * (windowrounding + windowborder_size), corner.y + grip.InnerDir.y * (windowrounding + windowborder_size)), windowrounding, grip.AngleMin12, grip.AngleMax12);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    const bool windowjust_created = (window == NULL);
    if (windowjust_created)
    ...
    bool windowjust_activated_by_user = (window->LastFrameActive < current_frame - 1);   // Not using !WasActive because the implicit "Debug" window would always toggle off->on
    const bool windowjust_appearing_after_hidden_for_resize = (window->HiddenFramesCannotSkipItems > 0);
    if (flags & ImGuiWindowFlags_Popup)
    {
        ImGuiPopupData& popup_ref = g.OpenPopupStack[g.BeginPopupStack.Size];
        windowjust_activated_by_user |= (window->PopupId != popup_ref.PopupId); // We recycle popups so treat window as activated if popup id changed
        windowjust_activated_by_user |= (window != popup_ref.Window);
    }
    window->Appearing = (windowjust_activated_by_user || windowjust_appearing_after_hidden_for_resize);
    if (window->Appearing)
    ...
    ImGuiWindow* parent_windowin_stack = g.CurrentWindowStack.empty() ? NULL : g.CurrentWindowStack.back();
    ImGuiWindow* parent_window = first_begin_of_the_frame ? ((flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_Popup)) ? parent_windowin_stack : NULL) : window->ParentWindow;
    ...
    if (windowjust_appearing_after_hidden_for_resize && !(flags & ImGuiWindowFlags_ChildWindow))
    ...
    bool windowpos_set_by_api = false;
    bool windowsize_x_set_by_api = false, windowsize_y_set_by_api = false;
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos)
    {
        windowpos_set_by_api = (window->SetWindowPosAllowFlags & g.NextWindowData.PosCond) != 0;
        if (windowpos_set_by_api && ImLengthSqr(g.NextWindowData.PosPivotVal) > 0.00001f)
        {
    ...
    {
        windowsize_x_set_by_api = (window->SetWindowSizeAllowFlags & g.NextWindowData.SizeCond) != 0 && (g.NextWindowData.SizeVal.x > 0.0f);
        windowsize_y_set_by_api = (window->SetWindowSizeAllowFlags & g.NextWindowData.SizeCond) != 0 && (g.NextWindowData.SizeVal.y > 0.0f);
    ...
    {
        // Initialize
        const bool windowis_child_tooltip = (flags & ImGuiWindowFlags_ChildWindow) && (flags & ImGuiWindowFlags_Tooltip); // FIXME-WIP: Undocumented behavior of Child+Tooltip for pinned tooltip (#1345)
        window->Active = true;
        ...
        bool windowtitle_visible_elsewhere = false;
        if (g.NavWindowingListWindow != NULL && (window->Flags & ImGuiWindowFlags_NoNavFocus) == 0)   // Window titles visible when using CTRL+TAB
            windowtitle_visible_elsewhere = true;
        if (windowtitle_visible_elsewhere && !windowjust_created && strcmp(name, window->Name) != 0)
        {
        ...
        if (windowjust_created && (!windowsize_x_set_by_api || !windowsize_y_set_by_api))
            window->HiddenFramesCannotSkipItems = 1;

        // Hide popup/tooltip window when re-opening while we measure size (because we recycle the windows)
        // We reset Size/ContentSize for reappearing popups/tooltips early in this function, so further code won't be tempted to use the old size.
        if (windowjust_activated_by_user && (flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip)) != 0)
        {
            window->HiddenFramesCannotSkipItems = 1;
            if (flags & ImGuiWindowFlags_AlwaysAutoResize)
            {
                if (!windowsize_x_set_by_api)
                    window->Size.x = window->SizeFull.x = 0.f;
                if (!windowsize_y_set_by_api)
        ...
        bool use_current_size_for_scrollbar_x = windowjust_created;
        bool use_current_size_for_scrollbar_y = windowjust_created;
        ...
        else if (window->AutoFitFramesX > 0 || window->AutoFitFramesY > 0)
        {
            ...
            if (!windowsize_x_set_by_api && window->AutoFitFramesX > 0)
            {
                window->SizeFull.x = window->AutoFitOnlyGrows ? ImMax(window->SizeFull.x, size_auto_fit.x) : size_auto_fit.x;
                use_current_size_for_scrollbar_x = true;
            }
            if (!windowsize_y_set_by_api && window->AutoFitFramesY > 0)
            {
        ...
        if (windowjust_activated_by_user)
        {
            window->AutoPosLastDirection = ImGuiDir_None;
            if ((flags & ImGuiWindowFlags_Popup) != 0 && !(flags & ImGuiWindowFlags_Modal) && !windowpos_set_by_api) // FIXME: BeginPopup() could use SetNextWindowPos()
        ...
        if (flags & ImGuiWindowFlags_ChildWindow)
        {
            ...
            if (!(flags & ImGuiWindowFlags_Popup) && !windowpos_set_by_api && !windowis_child_tooltip)
                window->Pos = parent_window->DC.CursorPos;
        }

        const bool windowpos_with_pivot = (window->SetWindowPosVal.x != FLT_MAX && window->HiddenFramesCannotSkipItems == 0);
        if (windowpos_with_pivot)
            SetWindowPos(window, window->SetWindowPosVal - window->Size * window->SetWindowPosPivot, 0); // Position given a pivot (e.g. for centering)
        else if ((flags & ImGuiWindowFlags_ChildMenu) != 0)
            window->Pos = FindBestWindowPosForPopup(window);
        else if ((flags & ImGuiWindowFlags_Popup) != 0 && !windowpos_set_by_api && windowjust_appearing_after_hidden_for_resize)
            window->Pos = FindBestWindowPosForPopup(window);
        else if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !windowpos_set_by_api && !windowis_child_tooltip)
            window->Pos = FindBestWindowPosForPopup(window);
        ...
        if (!windowpos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0)
        ...
        if (windowjust_activated_by_user && !(flags & ImGuiWindowFlags_NoFocusOnAppearing))
        {
            if (flags & ImGuiWindowFlags_Popup)
                want_focus = true;
        ...
        if (!window->Collapsed)
        {
            ...
            ImVec2 needed_size_from_last_frame = windowjust_created ? ImVec2(0, 0) : window->ContentSize + window->WindowPadding * 2.0f;
        ...
        const ImRect host_rect = ((flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !windowis_child_tooltip) ? parent_window->ClipRect : viewport_rect;
        ...
        const bool dim_bg_for_windowlist = g.NavWindowingTargetAnim && (window == g.NavWindowingTargetAnim->RootWindow);
        if (dim_bg_for_modal || dim_bg_for_windowlist)
        {
        ...
        if (dim_bg_for_windowlist && window == g.NavWindowingTargetAnim)
        {
        ...
        // FIXME: More code may rely on explicit sorting of overlapping child window and would need to disable this somehow. Please get in contact if you are affected.
        {
            ...
            if ((flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !windowis_child_tooltip)
            ...
            const ImGuiWindow* windowto_highlight = g.NavWindowingTarget ? g.NavWindowingTarget : g.NavWindow;
            const bool title_bar_is_highlight = want_focus || (windowto_highlight && window->RootWindowForTitleBarHighlight == windowto_highlight->RootWindowForTitleBarHighlight);
        ...
        window->DC.MenuColumns.Update(3, style.ItemSpacing.x, windowjust_activated_by_user);
...
void ImGui::FocusTopMostWindowUnderOne(ImGuiWindow* under_this_window, ImGuiWindow* ignore_window)
{
    ...
    {
        int under_this_windowidx = FindWindowFocusIndex(under_this_window);
        if (under_this_windowidx != -1)
            start_idx = under_this_windowidx - 1;
    }
...
{
    ImGuiContext& g = *GImGui;
    ImRect windowrect(window->InnerRect.Min - ImVec2(1, 1), window->InnerRect.Max + ImVec2(1, 1));
    //GetForegroundDrawList(window)->AddRect(windowrect.Min, windowrect.Max, IM_COL32_WHITE); // [DEBUG]

    ImVec2 delta_scroll;
    if (!windowrect.Contains(item_rect))
    {
        if (window->ScrollbarX && item_rect.Min.x < windowrect.Min.x)
            SetScrollFromPosX(window, item_rect.Min.x - window->Pos.x - g.Style.ItemSpacing.x, 0.0f);
        else if (window->ScrollbarX && item_rect.Max.x >= windowrect.Max.x)
            SetScrollFromPosX(window, item_rect.Max.x - window->Pos.x + g.Style.ItemSpacing.x, 1.0f);
        if (item_rect.Min.y < windowrect.Min.y)
            SetScrollFromPosY(window, item_rect.Min.y - window->Pos.y - g.Style.ItemSpacing.y, 0.0f);
        else if (item_rect.Max.y >= windowrect.Max.y)
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, ImGuiTooltipFlags tooltip_flags)
{
    ...
    char windowname[16];
    ImFormatString(windowname, IM_ARRAYSIZE(windowname), "##Tooltip_%02d", g.TooltipOverrideCount);
    if (tooltip_flags & ImGuiTooltipFlags_OverridePreviousTooltip)
        if (ImGuiWindow* window = FindWindowByName(windowname))
            if (window->Active)
            {
    ...
    Begin(windowname, NULL, flags | extra_flags);
}
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window, bool restore_focus_to_windowunder_popup)
{
    ImGuiContext& g = *GImGui;
    ...
    if (ref_window)
    {
        ...
        for (; popup_count_to_keep < g.OpenPopupStack.Size; popup_count_to_keep++)
        {
            ...
            bool ref_windowis_descendent_of_popup = false;
            for (int n = popup_count_to_keep; n < g.OpenPopupStack.Size; n++)
                if (ImGuiWindow* popup_window = g.OpenPopupStack[n].Window)
                    if (popup_window->RootWindow == ref_window->RootWindow)
                    {
                        ref_windowis_descendent_of_popup = true;
                        break;
                    }
            if (!ref_windowis_descendent_of_popup)
                break;
        }
    }
    if (popup_count_to_keep < g.OpenPopupStack.Size) // This test is not required but it allows to set a convenient breakpoint on the statement below
    {
        ...
        ClosePopupToLevel(popup_count_to_keep, restore_focus_to_windowunder_popup);
    }
}

void ImGui::ClosePopupToLevel(int remaining, bool restore_focus_to_windowunder_popup)
{
    ImGuiContext& g = *GImGui;
    IMGUI_DEBUG_LOG_POPUP("ClosePopupToLevel(%d), restore_focus_to_windowunder_popup=%d\n", remaining, restore_focus_to_windowunder_popup);
    ...
    if (restore_focus_to_windowunder_popup)
    {
...
static void ImGui::NavUpdate()
{
    ...
    {
        ImGuiWindow* window = g.NavWindow;
        ImRect windowrect_rel(window->InnerRect.Min - window->Pos - ImVec2(1, 1), window->InnerRect.Max - window->Pos + ImVec2(1, 1));
        if (!windowrect_rel.Contains(window->NavRectRel[g.NavLayer]))
        {
            IMGUI_DEBUG_LOG_NAV("[nav] NavMoveRequest: clamp NavRectRel\n");
            float pad = window->CalcFontSize() * 0.5f;
            windowrect_rel.Expand(ImVec2(-ImMin(windowrect_rel.GetWidth(), pad), -ImMin(windowrect_rel.GetHeight(), pad))); // Terrible approximation for the intent of starting navigation from first fully visible item
            window->NavRectRel[g.NavLayer].ClipWithFull(windowrect_rel);
            g.NavId = g.NavFocusScopeId = 0;
        }
    }
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir)
{
    ...
    ImGuiWindow* windowtarget = FindWindowNavFocusable(i_current + focus_change_dir, -INT_MAX, focus_change_dir);
    if (!windowtarget)
        windowtarget = FindWindowNavFocusable((focus_change_dir < 0) ? (g.WindowsFocusOrder.Size - 1) : 0, i_current, focus_change_dir);
    if (windowtarget) // Don't reset windowing target if there's a single window in the list
        g.NavWindowingTarget = g.NavWindowingTargetAnim = windowtarget;
    g.NavWindowingToggleLayer = false;
}
...
// Call registered handlers (e.g. SettingsHandlerwindowWriteAll() + custom handlers) to write their stuff into a text buffer
