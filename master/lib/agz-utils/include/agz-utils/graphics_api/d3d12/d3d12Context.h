...
public:
    ...
    AGZ_DECL_EVENT_SENDER_HANDLER(window, WindowCloseEvent)
    AGZ_DECL_EVENT_SENDER_HANDLER(window, WindowGetFocusEvent)
    AGZ_DECL_EVENT_SENDER_HANDLER(window, WindowLostFocusEvent)
    AGZ_DECL_EVENT_SENDER_HANDLER(window, WindowResizeEvent)
...
inline D3D12Context::D3D12Context(
    ...
    : window(windowDesc),
      ...
      swapChain_(swapChainDesc, window, device_),
      ...
      rscMgr_(device_, device_.getAdapter())
{
    ...
    {
        imgui_.initialize(
            window, swapChain_, device_, descAlloc_.allocStatic());
    }
}
